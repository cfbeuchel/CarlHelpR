...
venndiagram <- function(x = x, y = y, z = z, w = w, unique = T, title = "Venn Diagram", labels = c("x", "y", "z", "w"), lines = 1, lcol = 1, tcol = 1, diacol = 1, plot = T, type = "3",
    ...
    if (type == "2") {
        ...
        olDF <- data.frame(x = c(5, 3.1, 7), y = c(6.1, 6.1, 6.1), count = countDF$count)
        if (printsub == TRUE) {
            mysub <- paste(paste("N unique: xy =", length(unique(c(x, y)))), paste("; x =", length(unique(x))), paste("; y =", length(unique(y))), sep = "")
        } else {
            mysub <- ""
        }
        if (plot == T) {
            ## Plot the 2-way venn diagram
            graphics::symbols(x = c(4, 6), y = c(6, 6), circles = c(2, 2), xlim = c(0, 10), ylim = c(0, 10), inches = F, main = title, sub = mysub, xlab = "", ylab = "", xaxt = "n", yaxt = "n",
                bty = "n", fg = lines, ...)
            graphics::text(olDF$x, olDF$y, olDF$count, col = tcol, ...)
            graphics::text(c(2, 8), c(8.8, 8.8), labels[1:2], col = lcol, ...)
        }
    ...
    if (type == "2map") {
        olDFdebug <- data.frame(x = c(5, 3.1, 7), y = c(6.1, 6.1, 6.1), count = paste("q", 1:3, sep = ""), ...)
        graphics::symbols(x = c(4, 6), y = c(6, 6), circles = c(2, 2), xlim = c(0, 10), ylim = c(0, 10), inches = F, main = "Mapping Venn Diagram", xlab = "", ylab = "", xaxt = "n", yaxt = "n",
            bty = "n", fg = lines, ...)
        graphics::text(olDFdebug$x, olDFdebug$y, olDFdebug$count, col = tcol, ...)
        graphics::text(c(2, 8), c(8.8, 8.8), paste(labels[1:2], "=", c("x", "y")), col = lcol, ...)
    }
    ...
    if (type == "3") {
        ...
        count <- unlist(lapply(qlist, length))
        countDF <- data.frame(query = names(count), count = as.vector(count))
        olDF <- data.frame(x = c(5, 3.8, 6.3, 5, 3, 7, 5), y = c(5.6, 4.6, 4.6, 6.9, 6.5, 6.5, 3), count = countDF$count)
        if (printsub == TRUE) {
            mysub <- paste(paste("N unique: xyz =", length(unique(c(x, y, z)))), paste("; x =", length(unique(x))), paste("; y =", length(unique(y))), paste("; z =", length(unique(z))),
                sep = "")
        } else {
            mysub <- ""
        }
        if (plot == T) {
            ...
            graphics::symbols(x = c(4, 6, 5), y = c(6, 6, 4), circles = c(2, 2, 2), xlim = c(0, 10), ylim = c(0, 10), inches = F, main = title, sub = mysub, xlab = "", ylab = "", xaxt = "n",
                yaxt = "n", bty = "n", fg = lines, ...)
            graphics::text(olDF$x, olDF$y, olDF$count, col = tcol, ...)
            graphics::text(c(2, 8, 6), c(8.8, 8.8, 1.1), labels[1:3], col = lcol, ...)
        }
    ...
    if (type == "3map") {
        olDFdebug <- data.frame(x = c(5, 3.8, 6.3, 5, 3, 7, 5), y = c(5.6, 4.6, 4.6, 6.9, 6.5, 6.5, 3), count = paste("q", 1:7, sep = ""), ...)
        graphics::symbols(x = c(4, 6, 5), y = c(6, 6, 4), circles = c(2, 2, 2), xlim = c(0, 10), ylim = c(0, 10), inches = F, main = "Mapping Venn Diagram", xlab = "", ylab = "", xaxt = "n",
            yaxt = "n", bty = "n", fg = lines, ...)
        graphics::text(olDFdebug$x, olDFdebug$y, olDFdebug$count, col = tcol, ...)
        graphics::text(c(2, 8, 6), c(8.8, 8.8, 1.1), paste(labels[1:3], "=", c("x", "y", "z")), col = lcol, ...)
    }
    ...
    if (type == "4" | type == "4el" | type == "4elmap") {
        ...
        count <- unlist(lapply(qlist, length))
        countDF <- data.frame(query = names(count), count = as.vector(count))
        olDF <- data.frame(x = c(4.8, 3.9, 5.7, 3.9, 5.7, 4.8, 4.8, 3, 6.5, 3, 6.5, 3, 6.5, 4.8, 4.8), y = c(5.2, 4.2, 4.2, 6.3, 6.3, 7.2, 3.2, 5.2, 5.2, 7.2, 7.2, 3.2, 3.2, 1,
            0.4), count = countDF$count)

        if (printsub == TRUE) {
            mysub <- paste(paste("N unique: xyzw =", length(unique(c(x, y, z, w)))), paste("; x =", length(unique(x))), paste("; y =", length(unique(y))), paste("; z =", length(unique(z))),
                paste("; w =", length(unique(w))), sep = "")
        } else {
            mysub <- ""
        }
        ...
        if (plot == T & type == "4") {
            graphics::symbols(x = c(4, 5.5, 4, 5.5), y = c(6, 6, 4.5, 4.5), circles = c(2, 2, 2, 2), xlim = c(0, 10), ylim = c(0, 10), inches = F, main = title, sub = mysub, xlab = "",
            ...
            graphics::text(c(2, 7.5, 2, 7.5), c(8.3, 8.3, 2, 2), labels, col = lcol, ...)
            graphics::text(c(3.8, 3.8), c(1, 0.4), c(paste("Only in ", labels[1], " & ", labels[4], ": ", olDF$count[14], sep = ""), paste("Only in ", labels[2], " & ", labels[3], ": ",
                olDF$count[15], sep = "")), col = diacol, ...)
        }

        ## Plot 4-way venn diagram as ellipses
        if (plot == T & (type == "4el" | type == "4elmap")) {
            olDF <- data.frame(x = c(5, 4.2, 6.4, 3.6, 5.8, 2.9, 7.1, 3.1, 6.9, 1.5, 3.5, 6.5, 8.5, 5, 5), y = c(2.8, 1.4, 4, 4, 1.4, 5.9, 5.9, 2.2, 2.2, 4.8, 7.2, 7.2, 4.8, 0.7,
                6), count = countDF$count)
            ## Plot ellipse
            ...
            if (type == "4elmap") {
                olDFdebug <- data.frame(x = c(5, 4.2, 6.4, 3.6, 5.8, 2.9, 7.1, 3.1, 6.9, 1.5, 3.5, 6.5, 8.5, 5, 5), y = c(2.8, 1.4, 4, 4, 1.4, 5.9, 5.9, 2.2, 2.2, 4.8, 7.2, 7.2,
                  4.8, 0.7, 6), count = paste("q", 1:15, sep = ""), ...)
    ...
    if (type == "4map") {
        olDFdebug <- data.frame(x = c(4.8, 3.9, 5.7, 3.9, 5.7, 4.8, 4.8, 3, 6.5, 3, 6.5, 3, 6.5, 4.8, 4.8), y = c(5.2, 4.2, 4.2, 6.3, 6.3, 7.2, 3.2, 5.2, 5.2, 7.2, 7.2, 3.2, 3.2,
            1, 0.4), count = paste("q", 1:15, sep = ""), ...)
        graphics::symbols(x = c(4, 5.5, 4, 5.5), y = c(6, 6, 4.5, 4.5), circles = c(2, 2, 2, 2), xlim = c(0, 10), ylim = c(0, 10), inches = F, main = "Mapping Venn Diagram", xlab = "",
            ylab = "", xaxt = "n", yaxt = "n", bty = "n", fg = lines, ...)
        graphics::text(olDFdebug$x[1:13], olDFdebug$y[1:13], olDFdebug$count[1:13], col = tcol, ...)
        graphics::text(c(2, 7.5, 2, 7.5), c(8.3, 8.3, 2, 2), paste(labels, "=", c("x", "y", "z", "w")), col = lcol, ...)
        graphics::text(c(3.8, 3.8), c(0.97, 0.36), c(paste("Only in ", labels[1], " & ", labels[4], ": ", olDFdebug$count[14], sep = ""), paste("Only in ", labels[2], " & ", labels[3],
